...
module.exports.run = async (bot, message, args) => {
    // if (!args[3]) return message.channel.send("Le nombre d'arguments est invalide (min: 4)");
    await message.channel.messages.fetch(args[0]).then(Rmsg => {
        Rmsg.react(args[1]);
        let args_f = args.filter(function (value, index, arr) { return value != ""; });

        reactJson = db.get("reactcollectors", Rmsg.channel.id, Rmsg.id);
        if (reactJson === false) {
        ...
        db.set("reactcollectors", Rmsg.channel.id, Rmsg.id, reactJson);
    }).catch(err => {
